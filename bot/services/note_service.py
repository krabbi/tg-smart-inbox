...
class SavedNote:
    """Result of saving a note — the persisted Item plus indexing status."""
...
class NoteService:
    """Save note items to the database and attempt semantic indexing."""
    ...
    async def save(self, text: str, user_id: int) -> SavedNote:
        """Persist a note Item and attempt to index it; return SavedNote with indexing status.

        The save itself always succeeds. Embedding is best-effort: when the
        Voyage AI key is missing or the call fails, the note is still persisted
        ...
        return SavedNote(item=item, indexed=indexed)
    ...
    async def _try_index(self, item: Item) -> bool:
        ...
        try:
            ...
            logger.exception("Embedding generation raised for note item %s", item.id)
            return False
        if vector is None:
            return False
        try:
            ...
            logger.exception("Failed to persist embedding for note item %s", item.id)
