...
| File | Responsibility |
|------|---------------|
| `item_repository.py` | CRUD for `Item` records; includes system-only `get_missing_summary` (link items with `scraped_text` but no `summary`) and `update_summary` (persists a generated summary) used by the backfill scheduler job. User-scoped pagination via `get_recent`, `get_recent_by_type`, `count_by_user`, `count_by_user_and_type`; full-text search via `search(user_id, query, *, limit, offset)` and `count_search(user_id, query)`; user-scoped deletion via `delete_for_user(item_id, user_id)` and `bulk_delete_for_user(item_ids, user_id)`. |
...
| `frontend/src/Model.ts` | Defines `AuthState` (`unauthenticated` / `authenticated`), `Route` (`"all" \| "tasks" \| "notes" \| "links" \| "ideas" \| "reminders"`), `DeleteConfirmState` discriminated union (`none` / `single` / `bulk`), and `ItemListState` interface (`items`, `page`, `total_pages`, `isLoading`, `searchQuery`, `selectedItemId`, `checkedIds`, `deleteConfirm`, `loadedDetail`, `feedback`). Top-level `Model` interface has fields `auth`, `currentRoute`, `isLoading`, `error`, and `itemList: ItemListState`. Exports `resetItemListState()` helper to return a blank `ItemListState`. |
| `frontend/src/Messages.ts` | `Msg` union type — every event is a plain tagged-union value. Auth/navigation messages: `TelegramLoginSuccess`, `AuthSuccess`, `AuthFailed`, `SessionExpired`, `AccessDenied`, `MeLoaded`, `NavigateTo`, `ClearError`. Item-list messages: `ItemsLoaded`, `ItemLoaded`, `ItemsError`, `SelectItem`, `BackToList`, `SearchItems`, `ClearSearch`, `ChangePage`, `ToggleItemCheck`, `RequestDeleteItem`, `ConfirmDeleteItem`, `CancelDelete`, `ItemDeleted`, `RequestBulkDelete`, `ConfirmBulkDelete`, `ItemsBulkDeleted`, `ClearItemFeedback`. |
| `frontend/src/views/Layout.ts` | Top-level shell view. Unauthenticated → delegates to `Login.view(model)` (Telegram Login Widget card). Authenticated → sidebar with category tabs + `#view-slot` content area, delegating to `ItemList` or `ItemDetail` for item routes and a placeholder for the reminders route. Exports `view(model)` (returns HTML string) and `attachListeners(root, dispatch, model)` (wires click handlers and, when unauthenticated, registers the `window.onTelegramAuth` callback via `Login.attachLoginListeners`). |
| `frontend/src/views/Login.ts` | Unauthenticated landing page. Renders the Telegram Login Widget card. Exports `view(model)` (HTML string) and `attachLoginListeners(dispatch)` (registers `window.onTelegramAuth`). |
| `frontend/src/views/ItemList.ts` | Paginated item list for routes `all \| tasks \| notes \| links \| ideas`. Renders item cards (title/preview, type badge, date), search bar, prev/next pagination, single-item delete (with confirmation dialog), bulk-delete via checkboxes (with confirmation dialog and "Deleted N items" feedback), empty state, and nothing-found state. Exports `view(model)` and `attachListListeners(root, dispatch)`. |
| `frontend/src/views/ItemDetail.ts` | Full item detail panel. Shows title, type badge, `created_at`, full content, description, and `scraped_text` (Link items only). Back button returns to the list. Exports `view(model, loadedItem)` and `attachDetailListeners(root, dispatch)`. |
